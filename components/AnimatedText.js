          ...
          return ( <div key={ `word-${ word }-${ i }` } style={ { display: 'inline-block' } }>
            {
              ...
              Array.from( word ).map( ( letter, i ) => {
                ...
                return (
                  <motion.span whileHover={ { textShadow: 'none', color:'#D6DADF30' } } transition={ { ease: "easeOut", duration: .5 } } key={ `${ i }` } style={ { position: 'relative', display: 'inline-block', width: 'auto' } } >
                    { letter }
                  </motion.span>
                )
...
const ShadedText = styled.div`
  ...
  text-shadow: 0px 12px 23px rgba(169, 180, 203, 0.2), 0px 4px 4px rgba(0, 0, 0, 0.15), 0px 3px 0px #D6DADF50;
  font-weight: 500;
  font-style: normal;
  letter-spacing: 0px;
  line-height: 1.2;
